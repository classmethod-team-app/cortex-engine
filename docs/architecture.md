...
graph TB
    ...
    MP -- "cortex 配布<br/>（ref: stable＝安定・各自1回インストール）" --> CONF
...
### 5.1.1 メンバーの導入体験（初回だけ手動インストール）

部の全メンバー展開を想定し、メンバーの手数を最小にすることを設計要件とする。settings.json 方式での体験は次のとおり。

- **初回**: 手元で `claude plugin marketplace add classmethod-team-app/cortex-engine@stable` → `claude plugin install cortex@cortex-engine` を実行する（デスクトップアプリは GUI でも可）。**外部ソースのプラグインは、リポの `enabledPlugins` 宣言だけではロードされない**（サプライチェーン対策として厳格化された Claude Code の現行仕様）。プラットフォーム側の扱いが変われば、この手順は不要になる
- **2 案件目以降**: プラグインの状態はユーザー単位（`~/.claude/plugins/`）で管理されるため、**インストールは 1 人 1 回きり**。別案件のリポを開いても同じプラグインがそのまま使われる。案件が増えてもメンバーの作業は増えない
- **クラウド実行では使えない**: Web版（claude.ai/code）・デスクトップアプリの Cloud セッションではプラグインが読み込まれず、効くのは**リポジトリに入っているもの**（`.claude/`・`.mcp.json`・`CLAUDE.md`）だけ。スキルを使う作業は手元（CLI / VS Code / デスクトップの Local セッション）で行う
...
メンバーへの案内は `docs/onboarding.md`（環境の選び方・導入手順・MCP 接続）に集約する。private マーケットプレイスのバックグラウンド自動更新には環境変数の認証トークンが要るが、未設定でも手動更新はできる（保存場所は動作環境で変わる。`docs/credentials.md` 参照）。
...
- リポの `.claude/settings.json` の `extraKnownMarketplaces` / `enabledPlugins`（`"plugin@marketplace": true` 形式）は、案件が前提とするプラグインの**宣言**。**外部ソースのプラグインは各メンバーが `claude plugin install` するまでロードされない**（サプライチェーン対策として厳格化。実測と公式ドキュメントで確認・2026-07-25）
- **クラウド実行（Web版・デスクトップの Cloud セッション）ではプラグインが利用できない**。効くのはリポ同梱の設定（`.claude/`・`.mcp.json`・`CLAUDE.md`）のみ（実測・2026-07-25）
