...
| デザイン画面（`design`） | `design:{fileKey}:{nodeId}` | `design:abc123XYZ:1023:456` | 2層構成: **目録は `デザイン/inventory/`（Silverミラー・frontmatterなし）**、**育成ノートは `Cortex/デザイン/`（Gold・frontmatterあり）**。両者は同じ安定IDを共有し、育成ノートが存在すればそのIDは実在解決される |
