...
function llmExtractTerms(source, existingTermTitles, excludedList) {
  ...
  const prefix = [
    ...
    "",
    "**定義が宣言されている語だけ**を登録する。「〜を〜と呼ぶ」「〜とは〜のこと」「〜を〜と定義する」のように、",
    "その語の意味を説明する記述がソースにあるものが対象。**その語が使われているだけ**のものは、前後から意味を",
    "推測できても登録しない（推測で定義を書かない）。",
    "とくに一般的な比喩表現（天井・床・地図・器・肌感 等）は、会話でそう表現されただけで定義された語ではないので",
    "登録しない。「品質の天井を上げる」のような使用から「天井とは品質の上限のこと」という定義を作らない。",
    "",
