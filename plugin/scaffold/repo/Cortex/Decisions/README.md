...
> **決定を変えるときは、既存の記録を書き換えない。** 訂正・撤回・方針変更は**新しいDecisionを起こし**、`relations` の `supersedes` で古い記録のIDを指す。Decisionsは「いつ・誰が・なぜ決めたか」の履歴であり、過去の判断を消すと経緯が追えなくなる。既存レコードに手を入れてよいのは、誤字・リンク切れの修正と `status` の昇格（`draft` → `active`）だけ。
...
| deciders | 決定に関わった人のリスト（`組織_名前` の形式、例: `CM_鈴木花子`, `{{クライアント名}}_田中太郎`）。人名は `Cortex/Members/records/` の `title`（正式表記）に揃える。名簿に無い人物は表記が確定していないので `CM_佐藤（要確認）` のように `（要確認）` を付ける |
...
本リポジトリは顧客が直接アクセスする前提で運用する。**売上・原価・見積・単価・アサイン工数・人事評価・顧客/ベンダーへの率直な評価といった社内限定の情報は書かない**（背景・理由の本文にも混ぜない）。判断に迷う情報はそもそも置かないこと。
