...
変更が必要な場合は、仕組みの正本である [cortex-engine](https://github.com/classmethod-team-app/cortex-engine) と [cortex-tools](https://github.com/classmethod-team-app/cortex-tools)（viewer / infra）をセットで計画すること（本構造は全案件が前提とするため、エンジン側で変更し配布する）。
...
- 手で直接書くのではなく、原則としてスキル（`/update-decision` `/update-glossary` 等）経由で生成・更新する
