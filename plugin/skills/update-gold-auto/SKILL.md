...
description: >-
  ...
  Cortex/Decisions/・Cortex/Glossary/・Cortex/Members/ へフェーズごとに逐次コミットで自動追記する（人手承認なし・夜間cron想定）
---
...
> 日報・週報は本スキルの責務ではありません（レポートは PMハーネスが Slack に配信します。Gold層 `Cortex/` に書き込めるのはエンジンの精製ワークフローだけ、という境界の規律に従い、本スキルは Decisions・Glossary・Members のみを書きます）。
