...
> 日報・週報は本スキルの責務ではありません（レポートは PMハーネスが Slack に配信します。Gold層 `Cortex/` に書き込めるのはエンジンの精製ワークフローだけ、という境界の規律に従い、本スキルは Decisions・Glossary・Members・Rules のみを書きます）。
...
- **抽出対象**: 案件・業界固有の用語、社内略語、一般語だがこの案件で特別な意味を持つ語、案件で使う外部サービス・技術・規格で説明が要るもの。**案件固有語は定義が情報源に明示されている語のみ**登録する（推測で定義を書かない・`references` は付けない）。一般公開のサービス・技術・標準語は Web ツールが使えるなら公式一次情報で定義し `references` にURLを記す（**Webツールが使えない実行環境では定義を書かず候補に留める**）。
...
- **用語より一段保守的に判断する**: Rule は AI の行動を直接制約する（禁止された曜日にデプロイ手順を案内しない等）ため、誤起票のコストが用語より高い。「今後も守る」「常に〜する」「〜は禁止」「原則〜する」等、その後もずっと効き続ける約束事だけを対象にする。**確信が持てなければ起票しない**。
