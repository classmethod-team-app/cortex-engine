...
- **抽出対象**: 案件・業界固有の用語、社内略語、一般語だがこの案件で特別な意味を持つ語。**定義が情報源に明示されている語のみ**登録する（推測で定義を書かない）。判断基準は「**その語の意味を知らないと、この案件のやり取りを誤読するか**」。用語集は語の意味を揃える場所であり、何を採用したか・何が存在するかを記録する場所ではない。
