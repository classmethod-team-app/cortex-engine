...
- **定義が宣言されている語だけ登録する**: 「〜を〜と呼ぶ」「〜とは〜のこと」のように意味を説明する記述が情報源にあるものが対象。**その語が使われているだけ**のものは、前後から意味を推測できても登録しない。とくに一般的な比喩表現（天井・床・地図・器・肌感 等）は、会話でそう表現されただけで定義された語ではないので登録しない（「品質の天井を上げる」という使用から「天井とは品質の上限のこと」という定義を作らない）。
