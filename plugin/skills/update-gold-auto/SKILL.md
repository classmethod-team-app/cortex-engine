...
- **抽出対象**: **定義が情報源に明示されている語のみ**登録する（推測で定義を書かない）。線引きは「一般語かどうか」ではなく「**業務ドメインの語彙か、実装・運用の道具か**」。用語集は語の意味を揃える場所であり、何を採用したか・何が存在するかを記録する場所ではない。
  - ⭕️ **業務ドメインの語彙**（システムが扱う対象・状態・イベント・役割）— 一般語であっても、属性・ライフサイクル・適用ルールが案件ごとに定まるものは登録する。例) クーポン / スタンプカード / 会員 / 加盟店 / 失効 / 差戻し / 一次連携
  - ⭕️ 案件独自の言い回し・社内略語 — 例) 温度（この案件では「見込み客の購買意欲の段階」）
  - ❌ **実装・運用の道具**（それを作る手段）— 例) PostgreSQL / CloudWatch RUM（製品名・サービス名。採用した事実は Decision であって語彙ではない）/ daily-report（スキル名）/ config.json（ファイル名。リポジトリ名・ディレクトリ名も同様）/ 山田太郎（人名。メンバー名簿の領分）
  - ❌ 一般的な意味しか書けず、この案件での扱いが情報源から読み取れない語
  - **ドメイン語彙には `kind: domain` を付ける**（オントロジー規約が許す値は `domain` のみ）。`kind: domain` の用語を辿ると案件のドメイン関連図が機械的に得られる。それ以外の語には `kind` 行を付けない。
- **照合と絞り込み**: 既存の `title`/`synonyms` と一致または実質同義 → 新規ファイルを作らず該当ファイルへ **synonyms追記**。除外リスト該当 → スキップ。**確信が持てない語は登録しない**（直コミットされるため保守的に判断する）。
- **既存語に触れているソースは4つに判定する**: `same`（既存の定義で説明できている → 何もしない）/ `extend`（既存では説明しきれない新事実がある）/ `conflict`（既存を否定・変更する新事実がある）/ `new`（新規）。**用語は更新型（1つの正しい定義を保つ）なので、extend・conflict では既存レコードの本文を書き直す**（Decision と違い、矛盾する定義を並立させない）。旧版は git 履歴に残る。書き直したレコードは **`status` を `draft` に戻す**（内容が変わったので再レビューが必要）。
- **定義は育てるが膨らませない**: 書き直すときは**現在の定義の長さを目安とし、大きく超えない**。新しい内容を入れるために長さが必要なら、既存の記述をより抽象度の高い表現にまとめてその分を捻出する。事例を列挙して説明するのではなく、事例から言えることを書く。`description` 200字・本文 3000字を超えたら、**切り詰めずに報告する**（2つの概念が混ざっている / 用語ではなく解説文書になっている / 決定の経緯が混ざっている、のサイン。分割・移設を人に返す）。
