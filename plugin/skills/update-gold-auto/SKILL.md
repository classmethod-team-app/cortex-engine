...
（ステップ2で読んだソースの内容から1〜2段落。何が動き、何が決まり、何が議論中か。**新規に文章を考える**のはここだけ）
...
- 更新された議事録・課題・外部ソースの概況を1〜3行（件数と主なもの）
```

- **Phase A〜Cが全て追記なしでも、このフェーズは実行する**（対象ソースがあった夜の記録として `status: active` で生成する。概要には「何が更新され、なぜ昇格に至らなかったか」を書く。差分ソース自体が無い夜はAI実行ごとスキップされるため、このスキルが動いている時点で書くべき動きは必ずある）。
