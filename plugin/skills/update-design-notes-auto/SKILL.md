...
- **処理済みの画面を除外する**: 既に育成ノートが存在し、かつノートの最終コミット時刻がその画面のソース（inventory md / png）の最終コミット時刻より新しい画面は、処理済みとしてスキップする（判定例: `git log -1 --format=%ct -- <path>` の比較）。SINCEが失敗runで進まない間もこの除外で毎晩未処理分だけが選ばれ、バックログが15画面ずつ前進する（除外しないと毎晩同じ先頭15画面を選び直して収束しない）
- **1回の実行で最大15画面**まで。除外後の対象が15を超える場合は先頭15画面だけを処理し、**残りの画面IDを `.design-notes-overflow`（リポジトリルート）に1行1件で書き出す**。ワークフローがこのフラグを見て本runを失敗（赤）で終え、SINCEを進めないため、翌晩の増分窓に未処理画面が再び入り巻き取られる。フラグは `Cortex/` の外に置くのでコミット対象に含まれない
