...
`.rulesync/rules/overview.md` の「プロジェクト概要と目的」がテンプレ初期文のままでないか。

### 4. ルールファイル生成

`.claude/` が生成済みか（未生成なら `pnpm dlx rulesync@latest generate`）。

> 以下のツール固有チェック（5〜9）は `Cortex/Home.md` の `tools` 宣言で「該当」を判定する。該当しないツールは ➖。

### 5. 課題管理ツールとの同期

（`tools.課題管理 == backlog`）`課題管理/issues/` に同期済みデータがあるか（空なら → `/backlog-pull`）。Backlog 以外なら ➖。

### 6. GitHub Actions Secrets

`AWS_ROLE_TO_ASSUME`（ビューア/Bedrock用）と、`tools` に応じたツール用シークレットが設定済みか。`BACKLOG_API_KEY`/`BACKLOG_DOMAIN`/`BACKLOG_PROJECT_KEY`（`課題管理 == backlog`）、`FIGMA_TOKEN`（`デザイン == figma`）。CI 内では `secrets.X != ''` で有無を判定できる（`gh secret list` には権限が要る。確認できなければ「未確認」とする）。
...
（`tools.会議 == google-meet`）Google Meet/Drive 連携が設定され、`会議/` に自動取得した議事録があるか。Teams 等なら ➖。

### 10. README の案件化

`README.md` が「テンプレート」表記のまま（複製元のテンプレ説明のまま）でないか（→ `/setup-project` ステップ11）。
