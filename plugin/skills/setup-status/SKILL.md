...
- 手動記入用の `{{例: ...}}` の記入ガイド（`デザイン/DESIGN.md` の本文等）は対象外。DESIGN.md のフロントマター（デザイントークン）は sync-designs が自動生成するため人手記入は不要。
...
（`tools.会議 == google-meet`）`会議/ingest-config.json` の `enabled` が `true` か。有効なら、運用（**会議名の頭に案件キー**＋**`cortex-notetaker` bot の招待**）が案内済みか・`会議/` に自動取り込みされた文字起こし・議事録が入り始めているかも確認（cortex-notetaker 連携。中央の艦隊レジストリ登録は `/setup-project` ステップ12参照）。Teams 等なら ➖。
**前提はクラスメソッド側が主催する Google Meet のみ**（顧客主催の会議は自動では入らない＝手動取り込み運用の案件では `enabled: false` でも正常）。
...
- `Cortex/Home.md` の `tools` が既定（`課題管理: backlog` / `会議: google-meet` / `共有資料: google-drive` / `チャット: slack`（Teams 等は `teams`） / `デザイン: figma` / `開発: github`）以外の場合はその値に更新し、必要なら `/customize-tooling` を案内する
