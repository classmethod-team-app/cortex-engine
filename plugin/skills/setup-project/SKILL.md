...
**`meetingKey` に括弧は書かない**（括弧は会議名の側に付ける）。**`meetingKey` を書いた案件は艦隊キーでは当たらない**（打つものを1つに保つため）。合図が案件間で重複すると**両方が未仕分けに落ちる**ので、既存の合図と被らない値にする（重複の警告は中央 Apps Script の実行ログにしか出ず、案件側には届かない——**被らせない責任はこちらにある**）。
