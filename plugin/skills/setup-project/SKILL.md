...
ルーティングは**会議名に `[合図]` または `【合図】` が入っているか**だけで決まる（半角・全角どちらでも可・大小文字は問わない・会議名のどこにあってもよい）。

合図は既定で**艦隊レジストリのキー**（`cortex-tools/infra/config.ts` の `key`）。長くて会議名に打ちにくい案件だけ、`会議/ingest-config.json` の `meetingKey` に短い合図を書く。

```json
{ "enabled": true, "transcriptDir": "会議", "meetingKey": "sushiro" }
```

**`meetingKey` に括弧は書かない**（括弧は会議名の側に付ける）。空白を含む値・64文字超は使えず、艦隊キーに戻る。**`meetingKey` を書いた案件は艦隊キーでは当たらない**（打つものを1つに保つため）。合図が案件間で重複すると**両方が未仕分けに落ちる**ので、既存の合図と被らない値にする（重複の警告は中央 Apps Script の実行ログにしか出ず、案件側には届かない——**被らせない責任はこちらにある**）。

### 既に走っている定例（改名を頼みにくいもの）

**新規案件では使わない。** 以下は、Cortex を後から導入した案件で、既存の定例に合図を付けてもらうのが難しい場合の逃げ道。

`会議/ingest-config.json` の `aliases` に、**いまの会議名をそのまま**並べる。

```json
{ "enabled": true, "meetingKey": "sushiro", "aliases": ["sushiro:ASAKAI（SUSHIRO）"] }
```

**会議名まるごとの一致**なので他案件の会議を巻き込まない（廃止した `meetingNamePatterns` は素の部分一致で、短い語が他案件に反応した）。大小文字・前後の空白・`: / \ * ? " < > |` の書き換えは同一視する。定例はシリーズ名が固定なので、**1回登録すれば以後ずっと自動**。未仕分けに落ちたファイル名を写しても当たる（`:` が `_` に置換されている差、空白・大小文字の違いは吸収する）。

**改名を頼めるなら合図のほうがよい**。目印が1案件1つに保たれ、会議名を見た人が「どの案件か」を読み取れる。

顧客名・会議名パターンによる照合は**廃止した**。短いキーの素の部分一致が英単語に反応して誤配していたため（キー `ge` が `Gemini によるメモ` に一致し、艦隊の全文字起こしを1案件へ吸い込む状態だった）。

セットアップ手順:

1. `会議/ingest-config.json` の `enabled` を確認する。**既定は `true` で、そのままでよい**——`false` にするのは `Cortex/Home.md` の `tools.会議` が `google-meet` **でない**案件だけ（Teams 等。notetaker は Google Meet の文字起こししか拾えないため）。この2つは食い違わせないこと
...
4. **短い合図が要る案件だけ、`会議/ingest-config.json` に `meetingKey` を書く**（艦隊キーが長い場合。例: `sushiro-googlemaps` → `sushiro`）。既定のままでよければ書かない
5. **運用ルールをユーザーに案内**: 対象の会議名に**合図を括弧付きで入れ**（例:「[kc] 定例」「【kc】定例」）、**cortex-notetaker bot を会議の招待に追加**する（定例はシリーズに1回）。新しい定例を始めるPMに伝えることはこの2つだけ
   - **括弧の中身は合図そのもの**。`【KC定例】` のように他の文字が混ざると当たらない
   - **記号まで一致させる**。`_` と `-` は別物（`【TOKYU_LINE】` が `tokyu-line` で外れた実例がある）

> **`enabled` は「取り込みの可否」ではなく「振り分けの可否」**。取り込むかどうかは bot を会議に招待したかで決まるので、招待していなければ `true` でも何も起きない。**艦隊レジストリに登録していない案件は notetaker がそもそも読まない**ので、既定が `true` でも勝手に取り込まれることはない（登録が意思表示）。

**設定できたら1件で実際に確かめる。** この経路は各段が独立に失敗し、しかも**どれも「静かに未仕分けへ流れる」という同じ症状**になる（艦隊レジストリへの未登録・bot の権限不足・`enabled: false`・合図の不一致）。文字起こしが案件リポの会議ディレクトリに入るところまで見て初めて完了とみなす。
