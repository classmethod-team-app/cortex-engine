...
**`meetingKey` に括弧は書かない**（括弧は会議名の側に付ける）。**`meetingKey` を書いた案件は艦隊キーでは当たらない**（打つものを1つに保つため）。合図が案件間で重複すると**両方が未仕分けに落ちる**ので、既存の合図と被らない値にする（重複は取り込み時に警告が出る）。
...
4. **運用ルールをユーザーに案内**: 対象の会議名に**合図を括弧付きで入れ**（例:「[kc] 定例」「【kc】定例」）、**cortex-notetaker bot を会議の招待に追加**する（定例はシリーズに1回）。新しい定例を始めるPMに伝えることはこの2つだけ
