...
**完全一致**なので他案件の会議を巻き込まない（廃止した `meetingNamePatterns` は素の部分一致で、短い語が他案件に反応した）。定例はシリーズ名が固定なので、**1回登録すれば以後ずっと自動**。未仕分けに落ちたファイル名を写しても当たる（`:` が `_` に置換されている差、空白・大小文字の違いは吸収する）。
