...
この案件で使う職能ハーネス（PM / 開発 / デザイン / 運用保守 / 営業）をユーザーに確認し、使うものを `.claude/settings.json` の `enabledPlugins` に追記します:

```jsonc
"enabledPlugins": {
  "cortex@retail-app-harnesses": true,
  "pm-harness@retail-app-harnesses": true   // ← 使うハーネスだけ足す
}
```

これをコミットすると、メンバーがリポをトラストしたときに cortex＋選択したハーネスがまとめてインストール案内されます（案件としての宣言がリポに残り、後から参加するメンバーにも自動で行き渡る）。
...
## ステップ7: 初回の Backlog 全量同期（Actions で実行）

**ローカルに API キーを置く必要はありません。** Secrets 登録が済んだら、Actions 側に初回同期をさせます:
