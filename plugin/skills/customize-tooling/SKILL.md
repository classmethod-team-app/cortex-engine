...
---
Cortex テンプレは既定で **課題管理＝Backlog / デザイン＝Figma / 開発＝GitHub** を前提に配管が組まれています。案件が別ツール（例: Jira・Notion・Linear / Adobe XD・Penpot / GitLab 等）を使う場合に、**同じことを別ツールで実現する「設計 → 改善提案 → 実装」**までを対話的に行うスキルです。
...
承認後、チェックリストの全項目を反映する。

- 既定スキル／ワークフローを**複製して新ツール用に改修**（例: `backlog-pull` → `jira-pull`）。器の最小主義に従い、不要な作り込みはしない
- cortexプラグインの `.mcp.json`・Secrets 名・`.github/workflows/` を更新
- cortex-engine の `docs/ontology.md` の ID 規則を新ツールに合わせて更新（**型は維持**）
- `CLAUDE.md` / `README.md` / `setup-project` のツール参照・手順を更新（セットアップ手順の正本は `setup-project` スキルに集約されている点に注意）
- `node "<SKILL_DIR>/../../scripts/validate-cortex.mjs"`（プラグイン同梱リンター） で検証

### 6. 後始末

- 置き換えで不要になった旧ツール固有スキル・ワークフロー・Secrets の扱い（削除 or 無効化）をユーザーに確認する
