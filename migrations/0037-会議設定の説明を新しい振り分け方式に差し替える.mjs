...
const DOC =
  "顧客会議の文字起こしを Cortex へ自動取り込みする際の、この案件への『ルーティング設定』。" +
  "中央 Apps Script（cortex-notetaker bot 権限）は、bot に共有された文字起こしを“すべて”取り込み対象とし" +
  "（取り込むか否かは bot を招待したかどうかで決まる＝フィルタは持たない）、その文字起こしを" +
  "『どの案件リポへ入れるか』だけをここで判定する。判定は**会議名に [合図] または 【合図】 が入っているか**" +
  "だけを見る（半角・全角どちらでも可・大小文字は問わない）。合図は既定で艦隊レジストリのキー" +
  "（cortex-tools/infra/config.ts の key）。長くて会議名に打ちにくい場合だけ meetingKey に短い合図を書く" +
  "（例: sushiro-googlemaps → sushiro）。**meetingKey には括弧を書かない**（会議名の側に付ける。" +
  "「[kc]」のように書かれていた場合は剥がして扱い、警告を出す）。空白を含む値は使えない。" +
